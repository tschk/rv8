//! WebExtensions background script JS bridge (V8).
//!
...
impl BackgroundScriptRuntime {
    pub fn new(
        ...
        let handle = thread::spawn(move || {
            let mut engine = match JsEngine::new() {
                Ok(e) => e,
                Err(e) => {
                    warn!("Failed to create JsEngine for {}: {}", extension_id.0, e);
                    return;
                }
            };
            ...
            if let Err(e) = install_bridge(&mut engine, &mut state) {
                warn!(
...
fn install_bridge(engine: &mut JsEngine, state: &mut BridgeState) -> Result<(), String> {
    let handle_scope = &mut v8::HandleScope::new(&mut engine.isolate);
    let context = v8::Local::new(handle_scope, &engine.context);
